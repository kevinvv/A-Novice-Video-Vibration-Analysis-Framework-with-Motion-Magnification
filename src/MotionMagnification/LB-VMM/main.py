...
def amplify(model_path, input_video, output_video, *, amplification=1.0, device="cuda:0", skip_frames=1):
    device = torch.device(device)
    ...
    # Changed" Added static inputs
    amp_video.write_videofile(output_video, fps=50, codec='libx264', preset='slow', ffmpeg_params=['-pix_fmt','yuv420p'])
