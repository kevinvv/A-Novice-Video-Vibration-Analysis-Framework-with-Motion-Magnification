...
def saveVideo(video, saving_path, fps):
    ...
    temp_path = str.replace(saving_path, '.mp4', '_temp.mp4')
    ...
    Popen(f'ffmpeg -y -r {fps} -i {temp_path} -c:v libx264 -pix_fmt yuv420p -profile:v high -preset slow -crf 23 -an {saving_path}', shell=True).wait()  
